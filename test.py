...
def get_key_points(heatmap6, height, width):
	...
	:param heatmap6: The heatmap6 of CPM model.
...
def draw_image(image_path, key_points):
	"""
	Draw limbs in the image.
	:param image_path: The path of test image.
...
model = torch.load('cpm.pth').cuda()

image_path = 'test_data/test1.jpg'
image = cv2.imread(image_path)
height, width, _ = image.shape
image = np.asarray(cv2.imread(image_path), dtype=np.float32)
...
image = draw_image(image_path, key_points)
